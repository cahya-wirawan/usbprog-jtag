...
static unsigned int usbprog_hw_jtag_version = 1;
...
/* Global USB buffers */
static uint8_t usb_in_buffer[USBPROG_IN_BUFFER_SIZE];
static uint8_t usb_out_buffer[USBPROG_OUT_BUFFER_SIZE];
...
static int usbprog_speed_div(int speed, int *khz);
static int usbprog_khz(int khz, int *jtag_speed);
...
static void usbprog_path_move(int num_states, tap_state_t *path);
static void usbprog_runtest(int num_cycles);
static void usbprog_scan(int ir_scan, enum scan_type type, uint8_t *buffer, int scan_size, struct scan_command *command);
static void usbprog_reset(int trst, int srst);
static void usbprog_simple_command(uint8_t command,uint8_t _data);
static int usbprog_get_status(void);

/* USBProg tap buffer functions */
static void usbprog_tap_init(void);
static int  usbprog_tap_execute(void);
static void usbprog_tap_ensure_space(int scans, int bits);
static void usbprog_tap_append_step(int tms, int tdi);
static void usbprog_tap_append_scan(int length, uint8_t *buffer, struct scan_command *command);
...
static struct usbprog_jtag *usbprog_usb_open(void);
static void usbprog_usb_close(struct usbprog_jtag *usbprog_jtag);
static int usbprog_usb_message(struct usbprog_jtag *usbprog_jtag, int out_length, int in_length);
static int usbprog_usb_write(struct usbprog_jtag *usbprog_jtag, int out_length);
static int usbprog_usb_read(struct usbprog_jtag *usbprog_jtag);
//static int usbprog_usb_read_emu_result(struct usbprog_jtag *usbprog_jtag);
...
char time_str[50];
static void usbprog_debug_buffer(uint8_t *buffer, int length);
...
COMMAND_HANDLER(usbprog_handle_usbprog_info_command)
{
...
COMMAND_HANDLER(usbprog_handle_usbprog_hw_jtag_command)
{
	switch (CMD_ARGC) {
	case 0:
		command_print(CMD_CTX, "usbprog hw jtag  %i", usbprog_hw_jtag_version);
		break;
	case 1: {
		int request_version = atoi(CMD_ARGV[0]);
		switch (request_version) {
		case 2: case 3:
			usbprog_hw_jtag_version = request_version;
			break;
		default:
...
static const struct command_registration usbprog_command_handlers[] = {
	{
		.name = "usbprog_info",
		.handler = &usbprog_handle_usbprog_info_command,
		.mode = COMMAND_EXEC,
		.help = "show usbprog info",
	},
	{
		.name = "usbprog_hw_jtag",
		.handler = &usbprog_handle_usbprog_hw_jtag_command,
		.mode = COMMAND_EXEC,
		.help = "access USBProg HW JTAG command version",
		.usage = "[2|3]",
	},
...
{
	.name = "usbprog",
	.commands = usbprog_command_handlers,
	.execute_queue = usbprog_execute_queue,
	.speed = usbprog_speed,
	.speed_div = usbprog_speed_div,
	.khz = usbprog_khz,
	.init = usbprog_init,
	.quit = usbprog_quit,
};
...
void usbprog_scan(int ir_scan, enum scan_type type, uint8_t *buffer, int scan_size, struct scan_command *command)
{
...
void usbprog_simple_command(uint8_t command,uint8_t _data)
{
	int result;
...
static uint8_t tms_buffer[USBPROG_TAP_BUFFER_SIZE];
static uint8_t tdo_buffer[USBPROG_TAP_BUFFER_SIZE];
...
void usbprog_tap_append_scan(int length, uint8_t *buffer, struct scan_command *command)
{
...
struct usbprog_jtag* usbprog_usb_open()
{
	struct usb_bus *busses;
	struct usb_bus *bus;
	struct usb_device *dev;

	struct usbprog_jtag *result;

	result = (struct usbprog_jtag*) malloc(sizeof(struct usbprog_jtag));
...
void usbprog_usb_close(struct usbprog_jtag *usbprog_jtag)
{
...
int usbprog_usb_message(struct usbprog_jtag *usbprog_jtag, int out_length, int in_length)
{
	int result;
...
int usbprog_usb_write(struct usbprog_jtag *usbprog_jtag, int out_length)
{
	int result;
	...
	result = usb_bulk_write(usbprog_jtag->usb_handle, USBPROG_WRITE_ENDPOINT, \
		(char *)usb_out_buffer, out_length, USBPROG_USB_TIMEOUT);
#ifdef _DEBUG_USB_COMMS_
...
int usbprog_usb_read(struct usbprog_jtag *usbprog_jtag)
{
#ifdef _DEBUG_USB_COMMS_
	...
	int result = usb_bulk_read(usbprog_jtag->usb_handle, USBPROG_READ_ENDPOINT,
		(char *)usb_in_buffer, USBPROG_IN_BUFFER_SIZE, USBPROG_USB_TIMEOUT);
#ifdef _DEBUG_USB_COMMS_  
...
void usbprog_debug_buffer(uint8_t *buffer, int length)
{
	char line[81];
	char s[4];
	int i;
